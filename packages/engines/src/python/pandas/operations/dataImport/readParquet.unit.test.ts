...
describe("[Pandas] Data import operation: Read Parquet", () => {
    it("should handle happy path", async () => {
        await assertDataImportOperationCode(
            ...
            {
                runtimeCode: ["import pandas as pd", "df = pd.read_parquet(r'https://path/to/file.parquet')"].join("\n")
            }
        );
    });
    ...
    it("should handle happy path with single quote", async () => {
        await assertDataImportOperationCode(
            ...
            },
            {
                runtimeCode: [
                    "import pandas as pd",
                    "df = pd.read_parquet(r'https://path/to/\\'file\\'.parquet')"
                ].join("\n")
            }
        );
    });
    ...
    it("should handle happy path with relative uri", async () => {
        await assertDataImportOperationCode(
            ...
            },
            {
                ...
                displayCode: ["import pandas as pd", "df = pd.read_parquet(r'path/to/file.parquet')"].join("\n")
            }
        );
    });
});
