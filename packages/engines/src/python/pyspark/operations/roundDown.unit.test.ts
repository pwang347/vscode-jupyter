...
describe("[PySpark] Column operation: Round down column (floor)", () => {
        ...
        await assertOperationCode(
            ...
            },
            {
                code: [
                    ...
                    "df = df.withColumn('Some_column', F.floor(F.col('Some_column')))"
                ].join("\n")
            }
        );
    });
        ...
        await assertOperationCode(
            ...
            },
            {
                code: [
                    "from pyspark.sql import functions as F",
                    "df = df.withColumn('Some_column', F.floor(F.col('Some_column')))",
                    "df = df.withColumn('Another_column', F.floor(F.col('Another_column')))"
                ].join("\n")
            }
        );
    });
