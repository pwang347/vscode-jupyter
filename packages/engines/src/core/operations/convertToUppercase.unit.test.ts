import { ConvertToUppercaseOperationBase } from "./convertToUppercase";
import {
...
const operation = ConvertToUppercaseOperationBase();

describe("[Base Program] Column operation: Convert to uppercase", () => {
    it("should handle happy path for 1 selected column", async () => {
        await assertOperationBaseProgramGenSuccess(
            ...
            "Convert text to uppercase in column: 'Some_column'"
        );
    });
    ...
    it("should handle happy path for 2 selected columns", async () => {
        await assertOperationBaseProgramGenSuccess(
            ...
            "Convert text to uppercase in columns: 'Some_column', 'Another_column'"
        );
    });
