...
type IRoundDownOperationArgs = {
    TargetColumns: {
        value: IColumnTarget[];
    };
};
type IRoundDownBaseProgram = {
    variableName: string;
    columnKeys: string[];
};

/**
 * Base operation to round down numeric columns.
 */
export const RoundDownOperationBase: () => IGenericOperation<
    IRoundDownOperationArgs,
    IRoundDownBaseProgram,
            ...
            getDescription: (locale) =>
                ctx.formatString(
                    ctx.args.TargetColumns.value.length === 1
                        ? ctx.getLocalizedStrings(locale).OperationRoundDownDescription
                        : ctx.getLocalizedStrings(locale).OperationRoundDownDescriptionPlural,
